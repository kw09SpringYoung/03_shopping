...
class Cart extends React.Component {
    componentDidMount() {
        let { loadCarts, carts} = this.props
        // 向服务器发送请求获取购物车列表数据
        loadCarts()
    ...
    changeProductNumber(cid, e) {
        ...
        const { changServiceProductNumber} = this.props
        // 向服务端发送请求，告诉服务端哪一个商品的数量发生更改
        changServiceProductNumber({cid, count})
        ...
        <section className="container content-section">
                        ...
                        return (
                            <div className="cart-row" key={product.id}>
                                ...
                                <div className="cart-quantity cart-column">
                                    <input className="cart-quantity-input" type="number" value={product.count} onChange={(e)=>{
                                        this.changeProductNumber(product.id, e)
                                    }}/>
                                    <button className="btn btn-danger" type="button"
                                    onClick={ ()=> deleteProductFromCart(product.id)}>删除</button>
                                </div>
            ...
            <div className="cart-total">
                ...
                <span className="cart-total-price">￥{
                    carts.reduce((total, product)=>(total += product.count * product.price), 0)
                }</span>
            </div>
        </section>
        )
